...
- 系统会识别常见条款标题和条号，例如“保险责任”“责任免除”“等待期”“第十条”“10.1”等，并把这些结构化信息写入 chunk metadata，方便检索、展示和引用。
- 混合检索后会执行规则重排，优先提升标题命中、条号命中、主题匹配和关键保险术语匹配的片段，同时压低明显主题不一致的 hard negative 片段。

在 Web UI 的“检索依据详情”中，可以查看每个引用片段的向量分数、BM25 分数、最终融合分数、规则重排分数和重排依据。默认折叠这些细节，避免干扰普通问答流程。
...
系统还会做事实级证据核验：把答案中的具体事实拆成待核验断言，逐条检查是否能被用户保单引用支持。严重问题会阻断回答，轻微问题会在 UI 的“证据核验结果”折叠区展示风险提示。
...
合成评测使用仓库内置的安全样例，不依赖本地真实保单，适合在每次改动后快速检查 Recall@k、MRR、expected rank、引用覆盖和回答守卫行为。

可以运行合成 hard negative 评测，检查系统是否能避开“词很像但事实不对”的干扰片段：
...
如需对本地真实 PDF 做 hard negative 检查，可以运行：

```powershell
python scripts\evaluate_rag.py --local-hard-negative documents
```

本地 hard negative 评测会用同一批本地 PDF 构造正负查询对，例如区分“投保人豁免”和“被保险人豁免”，用于发现关键词相近但主体、责任或条件不同导致的误召回。
